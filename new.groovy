...
def linuxip = "192.168.1.6"
def macip = "192.168.1.74"
...
def emailNotification(buildstatus,sysdate,relnum){
    ...
    def binaryPath='\\'+'\\'+"${linuxip}"+'\\appzillontesting\\'+"${relnum}"+'\\'+"${sysdate}"
    if("${buildstatus}"=='Started'){
       ...
       emailBody= """<p>Hi All,</p><p><p>Machine Status: <br><br>Linux = <b>OK</b><br>Mac = <b>OK</b></p>AppzillonStudio-3.6.0 build triggered.<br>Please find the latest build details below.</p><p>Check console output at <a href='${env.BUILD_URL}'>${env.JOB_NAME}[${env.BUILD_NUMBER}]</a></p><p>Regards,<br>Appzillon DevTeam<br>I-Exceed Technology Solutions</p>"""
    }
    else if("${buildstatus}"=='Success'){
       recipentlist="preetish.das@i-exceed.com,prudhvi.kumar@i-exceed.com,darshanl.sagar@i-exceed.com,shyam.kumar@i-exceed.com,amar.khan@i-exceed.com,cc:chandu.balaji@i-exceed.com,cc:nagaraj.hiremath@i-exceed.com,cc:arthanarisamy@i-exceed.com,cc:prabudas.s@i-exceed.com" 
       ...
       recipentlist="preetish.das@i-exceed.com,darshanl.sagar@i-exceed.com,shyam.kumar@i-exceed.com,amar.khan@i-exceed.com,cc:chandu.balaji@i-exceed.com" 
...
pipeline { 
               ...
               parallel(
                  a: {
                    echo "JAR Distribution over linux server"
                     bat 'cd d:\\softs && plink -pw '+"${pass}"+' superuser@'+"${linuxip}"+' '+" -batch -hostkey "+"${key}"+" "+"mkdir /AppzillonBuilds/"+"${buildnumber}"
                     bat 'cd d:\\softs && pscp -r -pw '+"${pass}"+" -batch -hostkey "+"${key}"+" "+"${workspace}\\${defaulcotpath}\\appzillonstudio-client"+" "+"${workspace}\\${defaulcotpath}\\ClientJars"+" "+"${workspace}\\${defaulcotpath}\\KeyStore"+" "+"${workspace}\\${defaulcotpath}\\appzillonstudio-commandbuild\\target\\Appzillon"+' superuser@'+"${linuxip}"+':/AppzillonBuilds/'+"${buildnumber}"
                  },
                  b: {
                    echo "JAR Distribution over MAC"
                    bat 'cd d:\\softs && plink -pw '+"${Macpass}"+' admin@'+"${macip}"+' '+" -batch -hostkey "+"${Mackey}"+" "+"mkdir /AppzillonBuilds/"+"${buildnumber}"
                    bat 'cd d:\\softs && pscp -r -pw '+"${Macpass}"+" -batch -hostkey "+"${Mackey}"+" "+"${workspace}\\${defaulcotpath}\\appzillonstudio-client"+" "+"${workspace}\\${defaulcotpath}\\ClientJars"+" "+"${workspace}\\${defaulcotpath}\\KeyStore"+' admin@'+"${macip}"+':/AppzillonBuilds/'+"${buildnumber}"
                  }
                )
            }
             post {
    ...
    success{
        script{ def psid='"pgrep Appzillon | xargs kill -1"'
             bat 'cd d:\\softs && plink -pw '+"${Macpass}"+' admin@'+"${macip}"+' '+" -batch -hostkey "+"${Mackey}"+" "+"$psid"
        }
    }
  }
        }
        stage('Packaging') { 
            steps{
                parallel(
                  linux: {
                    echo "deb packaging"
                    bat 'cd d:\\softs && plink -pw '+"${pass}"+' superuser@'+"${linuxip}"+' '+" -batch -hostkey "+"${key}"+" "+'cd /AppzillonBuilds/'+"${buildnumber}"+'/appzillonstudio-client;ant'
                  },
                  window: {
                    echo "exe packaging"
                  ...
                  mac: {
                    ...
                    bat 'cd d:\\softs && plink -pw '+"${Macpass}"+' admin@'+"${macip}"+' '+" -batch -hostkey "+"${Mackey}"+" "+'cd /AppzillonBuilds/'+"${buildnumber}"+'/appzillonstudio-client;/Applications/apache-ant-1.9.4/bin/ant'
         ...
         post {
             success{
                 script{
                 sysdate="${nowdate}"
                 bat 'cd d:\\softs && plink -pw '+"${pass}"+' superuser@'+"${linuxip}"+' '+" -batch -hostkey "+"${key}"+" "+"mkdir -p /AppzillonTesting/"+"${params.AppzillonVersion}/"+"${sysdate}"
                 }
             }
    failure {
